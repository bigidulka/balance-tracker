...
async def test_speed():
    async with async_playwright() as p:
        ...
        # Wait until all 20 finish or timeout
        while len(captured) < 20 and time.time() - t0 < 3.0:
            await asyncio.sleep(0.01)
            
        print(f"Captured {len(captured)}/20 signatures in {time.time() - t0:.3f}s")
        if len(captured) > 0:
            print(f"Speed: {len(captured)/(time.time() - t0):.2f} sigs/sec")
            print(f"Equivalent to {int((len(captured)/(time.time() - t0)) * 3600)} signatures per hour")
