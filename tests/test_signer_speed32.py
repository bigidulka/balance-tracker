...
async def test_speed():
    async with async_playwright() as p:
        ...
        print("\nInjecting our custom requests (we WON'T abort them)...")
        t0 = time.time()
        
        captured.clear()
        
        # Test parallel injection but let them through so the app doesn't crash
        chains = ["eth", "bsc", "arb", "op", "matic", "avax", "ftm", "base", "linea", "scrl"]
        
        js_code = "() => {\n"
        for c in chains:
            url = f"https://api.debank.com/token/balance_list?user_addr={wallet.lower()}&chain={c}&_t={time.time()}"
            js_code += f"""
                var xhr_{c} = new XMLHttpRequest();
                xhr_{c}.open('GET', '{url}');
