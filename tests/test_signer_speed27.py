...
async def test_speed():
    ...
    print("Testing what DeBank deduplicates by. Is it just the path?")
    
    t0 = time.time()
    
    # We send requests to completely DIFFERENT wallets!
    for i in range(10):
        # Generate random wallet
        import random
        w = f"0x{''.join(random.choices('0123456789abcdef', k=40))}"
        url = f"https://api.debank.com/token/balance_list?user_addr={w}&chain=eth"
